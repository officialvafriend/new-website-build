...
let S={user:null,cart:[],orders:[],flt:'전체',seq:10482};
...
V.my=()=>{
 ...
 <div class="panel"><div style="display:flex;align-items:center;gap:.8rem">
  ...
  <span style="margin-left:auto;text-align:right"><b class="n" style="font-size:15px">2,400</b>
...
document.addEventListener('click',e=>{
 ...
 if(act==='leave'){
  modal('정말 탈퇴하시겠어요?',
   '<p>탈퇴하면 주문내역과 적립금 <b>2,400원</b>이 모두 사라지고 복구할 수 없습니다.</p>'+
   '<p style="margin-top:.6rem">배송 중인 주문이 있으면 탈퇴할 수 없습니다.</p>',
   '탈퇴하기',()=>{const busy=S.orders.some(o=>o.st==='ship');
    if(busy){toast('배송 중인 주문이 있어 탈퇴할 수 없습니다');return}
    S={user:null,cart:[],orders:[],flt:'전체',seq:S.seq};location.hash='#/';rerender();
    toast('탈퇴가 완료됐습니다')},true);
  return;
 }
});
