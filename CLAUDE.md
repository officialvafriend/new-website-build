...
- **하루 구매 한도 11병(10+1).** 담을 때(`woocommerce_add_to_cart_validation`) ·
...
- **병 수는 상품 이름에서 읽는다** (`Front\per_bottle()`). "10+1" 은 11병, "10병" 은
  10병, 이름에 수가 없으면 1병. 라인은 이름 앞 `[노보 블랙]` · `[노보]` 로 가른다 —
  **블랙을 먼저 봐야 한다**, 순서가 바뀌면 블랙이 일반으로 잡힌다
...
검증: `php design/php-tests/run.php` (노보 29개) · `scratchpad/live/novo1.mjs`(안내 ·
