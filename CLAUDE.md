...
### 자동 할인은 스니펫 토글로만 끌 수 있다 (2026-09-09, 재 봤다)

할인은 **사장님 Code Snippets** 가 붙인다 (쿠폰 목록에는 없다). 서버의 수수료 줄
(`🎁 금액 자동 할인`)과 결제 요약의 `쿠폰할인` 줄을 **한 스니펫이 둘 다** 만든다.

로그인 상태에서 워드커머스 수수료 줄만 걷어내 봤다 — **장바구니에 따라 갈린다.**
...
수수료 줄을 걷어내도 `쿠폰할인 -10,000원` 은 남는다 — 그 줄은 서버가 아니라 스니펫의
**JS** 가 상품가격을 보고 스스로 그린다. 테마는 `할인 = 상품가격 − 총액 − 쿠폰할인` 으로
되짚는데, 정가↔판매가 차이가 있는 상품은 그 차이가 흡수해 앞뒤가 맞고, 노보처럼
할인이 없는 상품은 `0 − 10,000 = -10,000` 이 되어 음수에서 총액 셈이 무너진다
(같은 화면에 `배송비 + 2,500원` 이 함께 나타나는 것도 같은 증상).

**그래서 서버의 줄과 스니펫의 JS 를 함께 꺼야 한다** — 관리자 → Snippets 에서 그
스니펫의 On/Off 토글 하나면 된다. 우리가 한쪽만 끄면 반드시 어긋나므로 플러그인에는
코드를 두지 않는다. 노보만 빼는 것도 같은 이유로 스니펫 안에서 해야 한다
(`design/노보-자동할인-제외-스니펫.md`). 시도한 코드: 커밋 `04fdf24` · `67159f7` · `84dbb33`.
