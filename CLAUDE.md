...
- **재구매는 읽어 둔 범위 안에서만 센다** (`duckhoo_sales_scan_days`, 400일). 그 전에 한 번
  사고 만 손님은 새 손님으로 잡힌다 — 화면 각주에 그 범위를 적어 둔다
- 10분 transient 캐시. `지금 다시 읽기` 는 논스를 낀 링크다. 상한
  `duckhoo_sales_max_orders`(8,000건)에 걸리면 각주가 알려 준다
