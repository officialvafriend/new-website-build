...
자동 반환을 짜려면 적립금을 더하는 쪽을 알아야 하는데 테마 · keyple 의 PHP 가
저장소에 없어서, 사이트 안에서 찾아 보여 준다:

- `woocommerce_order_status_cancelled` · `_refunded` · `_changed` 에 **무엇이 걸려 있는지**
  (여기에 적립금 함수가 없으면 그것이 곧 원인이다)
- 이름에 point/mileage/적립 이 든 **함수 · 메서드** (theme · keyple 것이 여기 나온다)
- 이름에 적립금이 든 **DB 표** (원장이 있는지)
- 최근 주문 중 적립금을 쓴 것 — 주문 번호 · 상태 · 회원 · 사용액 · 관련 메타 · 수수료 줄
- 그 회원의 적립금스러운 **user meta**

결과는 복사용 상자에 그대로 나온다. 다 쓰고 나면 이 파일과 `duckhoo-redesign.php` 의
`require` 한 줄을 지운다.
