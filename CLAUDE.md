...
노보는 물량이 모자라 값을 올린 상품이다. 거기에 10만원↑ 1만원 할인까지 얹히면 한 사람이
싸게 쓸어 가는 것을 우리가 거드는 셈이 된다.

**할인 자체를 없애지 않는다.** 노보 금액만 기준에서 빼고, 나머지가 기준을 넘으면 할인은
그대로 붙는다 (프로덕션에서 확인):

| 장바구니 | 할인 |
|---|---|
| 노보 135,000 만 | 없음 |
| 노보 135,000 + 다른 89,000 | 없음 (노보 뺀 기준 89,000) |
| 노보 135,000 + 다른 178,000 | **−10,000 그대로** |
| 노보 없음 | 손대지 않는다 |

할인은 쿠폰 플러그인이 **수수료 줄**(`🎁 금액 자동 할인`, 음수)로 붙인다.
`woocommerce_cart_calculate_fees` 우선순위 99 에서 그 줄만 다시 셈해 고치거나 뺀다
(`Novo\adjust_fees()`). 배송비 같은 다른 수수료 줄은 그대로 둔다. 규칙은
`Front\discount_tiers()` 한 곳이라 안내 문구와 서버 계산이 같은 값을 본다.
되돌리려면 `duckhoo_novo_exclude_from_discount` 를 false 로.

### 수량은 장바구니 수량이 아니다 (2026-09-08, 프로덕션 사고)

사장님 신고: **12병인데 결제가 됐다.** 재현해 보니 세는 곳이 통째로 틀려 있었다.

```
form.cart quantity = 1
wd_option_builder_json = [{"label":"노보 블랙 데저트","qty":12,"type":"required"}]
장바구니 줄: 수량 1 · 금액 162,000원 (13,500 × 12)
```

테마의 `wd-option-builder` 는 `quantity` 를 **1 로 두고** 실제 개수를 옵션 JSON 에 적는다.
장바구니 수량만 세면 12병이 1병이다. `?add-to-cart=…&quantity=11` 로만 시험해서
못 봤다 — **이 사이트에서 손님이 실제로 담는 길은 그쪽이 아니다.**

- `Novo\units_from_json()` — `type:required` 의 합이 그 상품의 **기본 단위 개수**다.
  단품이면 병 수, 묶음이면 세트 수. addon(맛 선택)은 그 안에 딸린 것이라 세지 않는다
- 장바구니 줄 · 주문 줄 메타 · 담기 요청(`$_POST`) 세 곳에서 모두 읽는다.
  **키 이름을 못 박지 않는다** — 값의 생김새(`group_key`)로 찾는다
- **워드프레스는 `$_POST` 의 모든 값에 역슬래시를 붙인다.** 그대로 `json_decode` 하면
  실패해 1병으로 돌아섰고, 고쳤다고 믿은 채로 12병이 또 통과했다. 실패하면
  `stripslashes()` 로 한 번 더 본다

### 검증 훅이 읽기 경로에도 걸린다 (같은 날, 같은 사고)

한도를 넘은 장바구니에서 `woocommerce_store_api_validate_cart_items` 가 예외를 던지자
**`GET /wc/store/v1/cart` 가 통째로 실패**했고, 테마의 결제 요약 스크립트가 금액을 못
읽어 **「총 주문금액 0원」**을 그렸다. 그 훅은 뺐다 — 막는 것은 상한(`store_max`)과
주문 만들기 직전(`guard_order`)이 한다. 둘 다 읽기를 깨뜨리지 않는다.

자동 할인 줄도 `remove_all_fees()` + `add_fee()` 로 다시 짓지 않는다. 쿠폰 플러그인이
그 줄에 달아 둔 값을 잃는다 — **금액만 제자리에서 0 으로 내린다.**

### 묶음의 「맛 몇 개」는 사장님 스니펫이 정한다

10+1 상품에서 맛이 `10 / 10` 으로 잡혀 11병을 못 고른다. 스니펫의 순서는:

1. 상품 메타 **「서랍장 필수 선택 수량」** (`window.vfDrawerConfig.requiredQty`) — 있으면 최우선
2. 없으면 PPOM 문구에서 `(\d+)\s*(?:가지|종|개입|병입|병)` 중 **가장 큰 수**

지금 이 상품은 1번이 `0` 이라 2번으로 떨어져 "10" 을 집는다. **상품 편집 화면의
「서랍장 필수 선택 수량」에 11 을 넣으면 된다** — 플러그인이 손댈 곳이 아니다.
(같은 스니펫의 `BUNDLE_RATIO` 는 이미 11 이다.)
