...
**리뷰 적립금은 아직 정해진 것이 없다.** 저장소 · 공지사항 · 상품 상세 · 안내 페이지를
다 찾아봤지만 액수를 적어 둔 곳이 없다 (2026-09-09 확인). 만들려면 금액을 먼저 정해야
하고, 지급은 `Points\return_points()` 와 같은 길로 **잔액(`_keyple_points`)과
원장(`wd_log_keyple_points_change`)을 둘 다** 건드려야 한다.
