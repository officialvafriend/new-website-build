...
### 낱병은 제한하지 않는다 (사장님 결정 2026-09-08)

10+1 묶음만 **하루 한 세트**. 낱병은 몇 병이든 산다. `duckhoo_novo_event` 의
`singles` 를 true 로 하면 낱병도 하루 10병이 된다. 로그인 상태에서 확인:

| 시도 | 결과 |
|---|---|
| 낱병 30병 | 담김 |
| 10+1 세트 1개 | 담김 |
| 다른 라인 세트 1개 더 | **거절** (일반 · 블랙 합산) |
| 그 뒤 낱병 20병 | 담김 |

### 고르는 자리에서 막아야 한다 (2026-09-08)

서버만 막으면 손님은 13개를 다 고른 뒤에야 거절을 만난다. 게다가 장바구니의
`옵션/수량 변경` 은 테마가 자기 길로 처리해 우리 검증을 지나지 않는다.

`front.js` 가 수량 `+` 를 capture 단계에서 가로채 오늘 살 수 있는 만큼에서 멈춘다.
남은 수량은 `window.DHR.novo` (필터 `duckhoo_js_config`). 확인함(프로덕션):

| 화면 | + 를 계속 눌렀을 때 |
|---|---|
| 단품 `[노보 블랙] 데저트` | **10병에서 멈춤** + 안내 |
| `[노보 리퀴드] 10+1` | **1세트에서 멈춤** + 안내 |

**단품과 묶음은 스테퍼가 다르다.** 단품은 테마의 `.wd-option-plus`, 묶음은 옛 옵션
UI 의 `.dhx-bundle .dhx-qty button` 이다. 둘 다 잡아야 한다 — 한쪽만 시험하면 다른
쪽이 통과한다.

### 「총 주문금액 0원」의 진짜 원인은 테마의 `$` 였다 (2026-09-08, 해결)

사장님 계정으로 결제 화면을 열어 보니 콘솔에 이것 하나가 떠 있었다:

```
TypeError: $ is not a function
```

테마의 `assets/js/wd-checkout-custom.js:185` 가 `jQuery(function($){…})` 래퍼 **밖에서**
`$` 를 쓴다. 거기서 죽고 그 아래가 통째로 안 돈다 — 합계를 내는 코드가 그 아래에 있다.
**저장소에 2026-09-02 부터 「알려진 문제」로 적혀 있던 그 버그다.** 쿠폰 체크박스가
안 먹던 것과 같은 원인이고, 노보 한도와는 무관했다.

`Shell\jquery_alias()` 가 jQuery 바로 뒤에 별칭 한 줄을 놓는다 (`duckhoo_jquery_alias`).
테마 파일은 건드리지 않는다. 고친 뒤 확인:

```
상품가격 135,000 · 배송비 무료 · 할인 -0 · 쿠폰할인 -10,000 · 총 주문금액 125,000원
```

**원인은 둘이었다.** `$` 를 고친 뒤 로그인 상태로 A/B 를 내 보니 우리 할인 손질도
같은 증상을 냈다 (노보만 135,000원 담은 장바구니):

```
할인 제외 꺼짐 → 총 주문금액 125,000원 · 할인 -0원      (정상)
할인 제외 켜짐 → 총 주문금액 0원 · 할인 - -10,000원     (깨짐)
```

이 가게의 자동 할인은 **수수료 줄로도 쿠폰으로도** 걸린다 (결제 요약에 `할인` 과
`쿠폰할인` 이 따로 있다). 수수료 줄만 0 으로 내리면 두 값이 어긋나 테마 요약이
무너진다. 그래서 `duckhoo_novo_exclude_from_discount` 는 **기본 꺼짐**이다 —
**노보를 자동 할인에서 빼려면 쿠폰 플러그인 설정(대상 분류에서 노보 제외)에서 한다.**

**교훈: 화면 숫자가 이상하면 먼저 콘솔을 본다. 그리고 A/B 로 가른다.**
추측으로 두 번(Store API 검사 · 결제 화면 안내) 헛짚었다 — 그 둘도 고칠 값은 있었지만
0원의 원인은 아니었다.

**로그인이 필요한 화면은 계정이 있어야 본다.** 이 사고는 비로그인으로는 끝까지
못 봤다 — `WP_USER`/`WP_PASS` 환경변수로만 쓰고 저장소에는 넣지 않는다.
