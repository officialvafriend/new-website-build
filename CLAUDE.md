...
**끄려면 `form-checkout.php:145~156` 의 「표시 안정화」 재계산 블록을 없애야 한다.**
그러면 결제 화면도 장바구니처럼 fee 를 합산하게 되어, 그 뒤로는 **우리 플러그인의
필터 한 줄**(`duckhoo_kill_auto_discount`)로 켜고 끌 수 있다. 그 블록은 애초에
같은 규칙을 두 번 적어 둔 것이라 없애는 편이 옳다 — 값이 갈릴 이유가 사라진다.

**그래서 `includes/discount.php` 는 스스로 지켜본다.** 매번 그 템플릿 파일을 읽어
`wd_tier_base` 가 있는지 보고 (`template_recomputes()`, 파일 mtime 으로 캐시 · 하루),
있으면 **아무것도 하지 않는다.** 사장님이 그 블록을 지우는 순간 자동으로 꺼진다 —
따로 배포하거나 눌러야 할 것이 없다. 파일을 못 읽으면 「아직 있다」로 본다
(모를 때는 건드리지 않는 쪽).

- 떼는 대상은 줄 번호가 아니라 **그 함수의 원본 코드에 `금액 자동 할인` 이 있는지**로
  찾는다 (`source_has()`, `ReflectionFunction`). 테마가 바뀌어도 엉뚱한 것을 떼지 않고,
  못 찾으면 할인이 그대로 살아 있을 뿐이다
- 끄면 `duckhoo_auto_discount` 규칙도 비워 안내 문구가 없는 혜택을 광고하지 않는다
  (`front.js` 가 장바구니 안내를 지운다)
- 아직 못 끄고 있는 동안에는 관리자에게 **왜인지 알림으로 말해 준다** — 조용히 안 되는
  것이 제일 나쁘다. 되살리려면 `duckhoo_kill_auto_discount` 를 false 로
- 안내 문서: `design/자동할인-끄기.md` (사장님께 보냄)
- 자리를 찾는 데 쓴 `도구 → 자동 할인 진단`(`includes/fee-doctor.php`)은 **지웠다.**
  다시 필요하면 git 이력에 있다
