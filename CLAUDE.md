...
### 홈 구성 (Apple Store 앱 참조)

큰 제목 → 분류 타일(가로 스냅) → 브랜드 칩 → "이번 달 볼 것" 검은 큰 카드 3개
(병이 스크롤 반대로 흐르는 패럴랙스, `scrub`) → 9월 특가 커버플로우 → 무니코틴
→ 구매 안내 3칸 → 푸터. 큰 제목이 헤더 뒤로 들어가면 헤더 가운데에 작은
제목이 켜진다 (`.gnb.small .ttl`).
