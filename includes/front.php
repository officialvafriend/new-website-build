 ...
 *
 * **지금 살아 있는 규칙은 10만원↑ 10,000원 한 단계뿐이다.** 사장님 스니펫의 글자에는
 * 아직 `5만원 이상 3,000원 / 8만원 이상 5,000원` 이 남아 있지만 그 이벤트는 끝났다
 * (사장님 확인 2026-09-08). 그래서 우리가 장바구니 안내를 다시 쓰는 것이 맞다 —
 * **스니펫의 글자를 그대로 믿고 여기 값을 늘리면 안 된다.** 한 번 그렇게 했다가
 * 끝난 이벤트를 도로 광고할 뻔했다.
 *
 ...
 *
 * **문구를 고른 기준 — 오해가 없어야 한다.**
 *
 * - **손님이 본 이름 그대로 쓴다.** 「금액 자동 할인」은 우리끼리 쓰는 말이고,
 *   띠에 떠 있던 것은 「10만원 이상 구매 시 10,000원 자동 할인」이었다
 * - **언제부터인지 적는다.** 어제 그 할인을 받은 손님이 자기가 잘못 본 줄 알면 안 된다
 * - **「그동안 이용해 주셔서 감사합니다」를 쓰지 않는다.** 쇼핑몰 맨 위 검은 띠에
 *   그 말이 있으면 **가게가 문을 닫는 것처럼** 읽힌다. 여기서 제일 큰 오해다
 * - 사과 · 인사는 넣지 않는다. 띠는 안내 한 줄이고, 폰에서 세 줄이 되면 화면을 먹는다.
 *   길게 쓸 말은 공지사항에 둔다
 *
