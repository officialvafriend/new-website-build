...
function seed(): string {
	return implode(
		...
		array(
			...
			'id:254',
			'',
			'# ↓ 가게에 그 이름이 없어서 못 찾은 줄입니다. 맞는 상품을 알려 주시면 `#` 만 지우면 됩니다.',
			'# 말라이트',
			'# 화이트멘솔          (비슷한 것: 화이트아웃 멘솔시가)',
			'# 디오알포',
			'# 블루워터            (비슷한 것: 디오리퀴드 블루마린 · 미슐랭 블루)',
			'# 타박멘솔            ([노보] 타박멘솔 은 있는데 [노보 블랙] 쪽엔 없습니다)',
			'# 엠에스블랜드',
			'# 라임알로에          (30ml 단품이 없고 [펠릭스] 모드 라임 알로에 60ml 만 있습니다)',
		)
	);
}
